...
jQuery(function($) {
    ...
    function adverts_upload_files_added(container, file) {
		...
		var numItems = $('.adverts-gallery-uploads .adverts-gallery-upload-item-img').length;
		 if(numItems >=1) {
		  $('.tt_upload_max').html('อัพโหลดได้สูงสุด 5 รูป');
		} else {
        ag_ui.append(ag_up);
    ...
    function adverts_upload_file_uploaded(file, result,container) {
       ...
       var numItems = $('.adverts-gallery-uploads .adverts-gallery-upload-item-img').length;
		 if(numItems >=1) {
		  $('.tt_upload_max').html('อัพโหลดได้สูงสุด 5 รูป');
		   var m = $("#"+file.id+" .adverts-gallery-upload-update");
            ...
            m.html($("<span></span>").addClass("adverts-gallery-upload-failed").html("กรุณากดลบรูปนี้"));
            return;
		   jQuery("#"+container+" .adverts-gallery-uploads").append(ag_ui);
		...
		var numItems = $('.adverts-gallery-uploads .adverts-gallery-upload-item-img').length;
		 if(numItems >=1) {
		 ag_p.append(ag_spin).append(ag_adel);
	...
	// --------------------------------------------------------------------------------------
		var numItems = $('.adverts-gallery-uploads .adverts-gallery-upload-item-img').length;
		 if(numItems >=1) {
		 $('.tt_upload_max').html('อัพโหลดได้สูงสุด 5 รูป');
		 $(".adverts-gallery-upload-update").remove();
