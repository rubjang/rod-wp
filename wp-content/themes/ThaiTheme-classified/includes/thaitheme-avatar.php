...
class Simple_Local_Avatars {
	...
	public function admin_init() {
		...
		add_settings_field( 'dw_focus-caps', __('Local Avatar Permissions','dw_focus'), array( $this, 'avatar_settings_field' ), 'discussion', 'avatars' );
	}
	...
	public function avatar_settings_field( $args ) {		
		...
		echo '
			<label for="simple_local_avatars_caps">
				...
				' . __('Only allow users with file upload capabilities to upload local avatars (Authors and above)','dw_focus') . '
			</label>
		';
	}
	...
	?>
	<h3><?php _e( 'อัพโหลดรูปโปรไฟล์ ','dw_focus' ); ?></h3>
		...
		<tr>
			...
			<?php
					if ( empty( $profileuser->simple_local_avatar ) )
						echo '<span class="description">' . __('อัพโหลดรูปโปรไฟล์','dw_focus') . '</span>';
					else 
						echo '
							<input type="checkbox" name="simple-local-avatar-erase" value="1" /> ' . __('ลบรูปภาพ','dw_focus') . '<br />
							<span class="description">' . __('* รูปจะต้องมีขนาด150x150 ขึ้นไป <br>* ชื่อรูปต้องเป็นภาษาอังกฤษหรือตัวเลขเท่านั้น <br>* หากต้องการเปลี่ยนรูปภาพใหม่ ให้เลือกไฟล์ภาพแล้วกด อัพเดทข้อมูลส่วนตัว','dw_focus') . '</span>
						';		
				} else {
					if ( empty( $profileuser->simple_local_avatar ) )
						echo '<span class="description">' . __('ไม่มีไฟล์ภาพ','dw_focus') . '</span>';
					else 
						echo '<span class="description">' . __('ไม่สามารถอัพโหลดรูปภาพได้กรุณาติดต่อผู้ดูแลเว็บไซต์','dw_focus') . '</span>';
				}
			?>
			</td>
		</tr>
			...
			if ( empty($avatar['file']) ) {		// handle failures
				switch ( $avatar['error'] ) {
					case 'File type does not meet security guidelines. Try another.' :
						add_action( 'user_profile_update_errors', create_function('$a','$a->add("avatar_error",__("Please upload a valid image file for the avatar.","dw_focus"));') );				
						break;
					default :
						add_action( 'user_profile_update_errors', create_function('$a','$a->add("avatar_error","<strong>".__("There was an error uploading the avatar:","dw_focus")."</strong> ' . esc_attr( $avatar['error'] ) . '");') );
				}
