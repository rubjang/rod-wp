...
<article class="tt_list thaitheme post_news <?php echo adext_adverts_css_classes( '', get_the_ID() ) ;?> <?php if($count % 2 == 0){echo " tt-2x";} if($count % 3 == 0){echo " tt-3x";} if($count % 4 == 0){echo " tt-4x";} if($count % 5 == 0){echo " tt-5x";} $count++; ?>" style="height:auto;">
<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>">
  <?php $image = thaitheme_img_list( get_the_ID() ) ?>
  <?php $image_new = substr($image,0,-12).".png";?>
  <?php if($image) { ?>
    <div class="tt-img">
    <?php $tt_sticky = adext_adverts_css_classes('', get_the_ID() ) ; if($tt_sticky !=="") { ?>
      <div class="tt-img-sticky"></div>
    <?php } ?>
      <img src="<?php echo $image_new; //esc_attr_e($image) ?>?v=1.0"  alt="<?php the_title(); ?>"/>
  ...
  <?php }else { ?>
    ...
    <?php $tt_sticky = adext_adverts_css_classes('', get_the_ID() ) ; if($tt_sticky !=="") { ?>
      ...
      <?php thaitheme_post_no_image(350,170); ?>
      <h3><?php esc_html_e( get_post( $post_id )->post_title ) ?></h3>
      <div class="tt-opacity"></div>
    </div>
  <?php } ?>
