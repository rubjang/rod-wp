...
<?php global $thaitheme_option;  $cat_link = $thaitheme_option['thaitheme_cat_ct8'];?>
<h2><a title="<?php echo get_cat_name($cat_link); ?>" href="<?php echo get_category_link($cat_link); ?>"><?php echo get_cat_name($cat_link); ?></a></h2>
</div>
<?php global $thaitheme_option; $thaitheme_shows_ct8 =  $thaitheme_option['thaitheme_shows_ct8']; $list_cat =  $thaitheme_option['thaitheme_cat_ct8'];$arr_id_cat_all=array(); foreach ((array) $list_cat as $id_cat_all ) {$arr_id_cat_all[]=$id_cat_all;}$my_query = new WP_Query( array( 'post_type' => 'post', 'category__in' => $arr_id_cat_all ,'posts_per_page' => ''.$thaitheme_shows_ct8.'', 'orderby' => array( 'menu_order' => 'DESC', 'date' => 'DESC' ), 'offset' => 0 ) );?>
