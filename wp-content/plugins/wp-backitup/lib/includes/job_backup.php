	...
	if (true===$WPBackitup->single_file_db()){
		...
		foreach ( $sql_files as $file ) {
			if ( false === $file_system->append_file_chunked($file,$toFile_path)){
				...
				WPBackItUp_LoggerV2::log_warning($backup_logname,$log_function, 'Could not merge SQL file:' . $file);
			};

			WPBackItUp_LoggerV2::log_info($backup_logname,$log_function, 'File Appended:' . $file);
		}
	}else{
...
//TODO:  Need to break this step into an iterative task that will add N SQL files per pass
...
if ('task_backup_db'==$current_task->getTaskName()) {
	WPBackItUp_LoggerV2::log($backup_logname,'**BACKUP DATABASE**' );
	...
	//Temp fix set batch limit to 10k
		...
		if ( $site_data_complete === false ) {
			...
			if ($site_data_complete>0){

				//See github issue 134
				//Should NOT get here because - temp fix set batch limit to 10k
				$current_task->setStatus(WPBackItUp_Job_Task::ERROR,105);
				...
				$wp_backup->cleanup_current_backup('txt|sql|db|config|crypt');
...
if ('task_create_manifest'==$current_task->getTaskName()) {
	WPBackItUp_LoggerV2::log( $backup_logname, '**CREATE BACKUP MANIFEST**' );
...
if ('task_encrypt_files'==$current_task->getTaskName()) {
	...
	WPBackItUp_LoggerV2::log($backup_logname,'**ENCRYPT SENSITIVE FILES**');
	...
	if (true===$WPBackitup->encrypt_files()){
		...
		if (count($file_list) != 1){
			WPBackItUp_LoggerV2::log_error($backup_logname,$log_function, 'More than 1 main File found:' . var_export( $file_list,true));
		...
		try {
			...
			WPBackItUp_LoggerV2::log_error($backup_logname,$log_function, 'Encryption Error:' .  $e);
			$encrypted=false;
		}

		if ( false === $encrypted){
			WPBackItUp_LoggerV2::log_error($backup_logname,$log_function, 'Could not encrypt file:' . var_export( $main_file,true));
		...
		WPBackItUp_LoggerV2::log_info($backup_logname,$log_function,'Encryption option off.');
	}

	set_status( 'encrypt', $complete, false );
	$current_task->setStatus(WPBackItUp_Job_Task::COMPLETE);
	WPBackItUp_LoggerV2::log($backup_logname,'**END ENCRYPT SENSITIVE FILES**');
