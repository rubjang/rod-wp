jQuery( function($) {
	...
	$(document).on( 'click', '.dpsp-tool-wrapper .dpsp-switch label', function(e) {
		...
		// Get location to activate/deactivate
		var location = $(this).siblings('input').data('location');

		if( location != 'undefined' ) {

			if( $actions_wrapper.hasClass('dpsp-inactive') ) {

				activateLocation( location ).done( function( response ) {
					if( response == 1 ) {
	...
	/*
	 * Make an AJAX call to activate a location
	 */
	function activateLocation( location ) {

		var data = {
			'action' 	: 'dpsp_activate_location',
			'dpsptkn'	: $('#dpsptkn').val(),
			'location'	: location
		}
	...
	/*
	 * Make an AJAX call to deactivate a location
	 */
	function deactivateLocation( location ) {

		var data = {
			'action' 	: 'dpsp_deactivate_location',
			'dpsptkn'	: $('#dpsptkn').val(),
			'location'	: location
		}
