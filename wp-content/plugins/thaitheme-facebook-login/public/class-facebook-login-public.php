...
class Facebook_Login_Public {
	...
	global $thaitheme_option; $thaitheme_select_post_member = $thaitheme_option['thaitheme_select_post_member'];
	if ( is_page($thaitheme_select_post_member) ) {
		...
		wp_localize_script( $this->plugin_name, 'fbl', apply_filters( 'fbl/js_vars', array(
			...
			'site_url'     => home_url('/?p='.$thaitheme_select_post_member.''),
		)));
	} else {
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/facebook-login.js', array( 'jquery' ), $this->version, false );
		wp_localize_script( $this->plugin_name, 'fbl', apply_filters( 'fbl/js_vars', array(
	...
	public function add_button_to_login_form() {
		echo apply_filters('fbl/login_button', '<a href="#" class="css-fbl js-fbl" data-fb_nonce="' . wp_create_nonce( 'facebook-nonce' ).'">'. __('เข้าสู้ระบบด้วย Facebook', $this->plugin_name) .'<img src="'.site_url('/wp-includes/js/mediaelement/loading.gif?v=1.0').'" alt=""/></a>');
	}
	...
	public function use_fb_avatars($avatar, $id_or_email, $size, $default, $alt ) {
		...
		if ( $user && is_object( $user ) ) {
			...
			if ( $fb_id = get_user_meta( $user_id, '_fb_user_id', true ) ) {
				...
				$fb_url = 'https://graph.facebook.com/' . $fb_id . '/picture?v=1.0?width=' . $size . '&height=' . $size;
